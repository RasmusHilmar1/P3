...
public interface MemberlistRepository extends JpaRepository<Member, Integer> {
    ...
    @Query("SELECT new com.example.p3.dto.MemberlistDTO(m.memberID, m.name, m.address, m.email, m.phonenumber, " +
            ...
            "CASE WHEN ab.boat IS NOT NULL THEN b.name ELSE 'Ingen båd' END, " + // Bådnavn er "No Boat", hvis båden ikke findes i Approvedboat
            ...
            "   CASE WHEN br.berthID = 9999 THEN 'Ingen plads' ELSE br.name END " +
            "   ELSE 'Ingen plads' END) " + // Berthnavn er "No Berth", hvis berth id er 9999 eller en båd ikke findes til medlemmet.
